...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            switch (choiceNumber) {
                case 1:
                    Customer customer = createAccountByCustomer();
                    do {
                        ...
                        switch (choiceNumber) {
                            case 1:
                                try {
                                    addToShoppingBag(customer);
                                } catch (Exception e) {
                                    System.out.println(e.getMessage());
                                }
                                break;
                            case 2:
                                confirmByCustomer(customer);
                                break;
                            case 3:
                                removeAnOrderByCustomer(customer);
                                break;
                            case 4:
                                break;
                            default:
    ...
    private static void confirmByCustomer(Customer customer) {
        String choice;
        int choiceNumber;
        List<Order> ordersList = orderService.getOrderList(customer);
        for (Order item : ordersList) {
            System.out.println(item);
        }
        System.out.println(orderService.sumOfOrderPrices(customer));
    ...
    private static void removeAnOrderByCustomer(Customer customer) {
        List<Order> orderList = orderService.getOrderList(customer);
        ...
        try {
            orderService.removeOrderFromBag(orderIdNumber);
    ...
    private static Customer createAccountByCustomer() {
        boolean error = true;
        String name = null;
        do {
            try {
    ...
    private static void addToShoppingBag(Customer customer) throws Exception {
        ...
        try {
            boolean isFull = orderService.addNewOrderToBag(Integer.parseInt(productId), Integer.parseInt(quantity),
                    java.sql.Date.valueOf(date), customer);
            if(isFull){
